	...
	describe('Standard Service Methods', () => {
		describe('create', () => {
			@suite class result extends ServiceClass {
				...
				@test(timeout(TIMEOUT)) async 'it create pem with provided attributes and settings' () {
			...
			describe('missing attributes', () => {
				@suite class result extends ServiceClass {
					...
					@test(timeout(TIMEOUT)) async 'it create pem with null attributes' () {
						let settings = { days: 364 }
			...
			describe('missing settings', () => {
				@suite class result extends ServiceClass {
					...
					@test(timeout(TIMEOUT)) async 'it creates pem with null settings' () {
						let attributes = { name: 'test' }
			...
			describe('invalid attributes', () => {
				@suite class result extends ServiceClass {
					...
					@test(timeout(TIMEOUT)) async 'it throws an error' () {
		...
		describe('get', () => {
			@suite class result extends ServiceClass {
				...
				@test(timeout(TIMEOUT)) async 'it returns a certificate by id' () {
					let pem = await this.get(this.testId)
					expect(pem.id).to.equal(this.testId)
				}
				@test(timeout(TIMEOUT)) async 'it supports $select' () {
		...
		describe('patch', () => {
			@suite class result extends ServiceClass {
				...
				@test(timeout(TIMEOUT)) async 'it regenerates pem by id, using current attributes and settings' () {
				...
				@test(timeout(TIMEOUT)) async 'it regenerates pems with new attributes' () {
				...
				@test(timeout(TIMEOUT)) async 'it regenerates pems with new settings' () {
		...
		describe('update', () => {
			@suite class result extends ServiceClass {
				...
				@test(timeout(TIMEOUT)) async 'regenerates pem by id, using new attributes and settings' () {
					let attributes = { name: 'new-attributes' }
					let settings = { days: 320 }
